...
- 검색된 문서(코사인 유사도 0.4 이상)만 근거로 답변하고, 없으면 지어내지 않고 상담 신청을 안내
- 서비스와 무관한 질문은 직접 답하지 않고 서비스 주제로 유도
- 법률·세무 판단은 하지 않고 전문가 상담을 권함
- 답을 못 찾거나 고객이 더 자세한 상담을 원할 때는 전화상담(성명+연락처를 채팅창에 남기기)을 함께 안내
- 임계값(`SIMILARITY_THRESHOLD`)과 모델(`LL_CHAT_MODEL`)은 `api/chat.js` 상단에서 조정 가능

## 챗봇 전화상담 요청 자동 등록 (`lib/contact-capture.js`)

챗봇 대화 중 고객이 전화번호가 포함된 메시지(예: "홍길동, 010-1234-5678")를 남기면:

1. 정규식으로 전화번호를 감지하고, 이름은 LLM으로 별도 추출합니다(이름을 못 찾으면 지어내지 않고 다시 요청).
2. 해당 세션(`chat_logs`)의 지금까지 대화를 불러와 "고객이 무엇을 궁금해했는지" 3~5문장으로 요약합니다.
3. `leads` 테이블에 `{name, phone, message: "[챗봇 전화상담 요청]\n"+요약}` 형태로 저장합니다 — 관리자 페이지 리드 탭과 엑셀 다운로드에 그대로 나타납니다.
4. 이 흐름은 문서 검색(RAG) 없이 즉시 처리되며, 정상적으로 접수됐다는 확인 메시지를 답합니다.

이름 없이 전화번호만 보내면 저장하지 않고 "성함도 함께 남겨달라"고 다시 안내합니다(대화 상태를 서버에 보관하지 않는 구조라, 이름과 번호를 한 메시지에 함께 받는 방식으로 설계했습니다).
